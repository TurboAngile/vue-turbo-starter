...
export default {
  classes: {
    wrapper ({ props }: ThemeParams) {
      ...
      // size
      if (props.size === 'xs') { classes.push('h-6 w-6 text-xs') } else if (props.size === 'sm') { classes.push('h-9 w-9 text-sm') } else if (props.size === 'lg') { classes.push('h-12 w-12 text-lg') } else if (props.size === 'xl') { classes.push('h-[3.75rem] w-[3.75rem] text-xl') } else { classes.push('h-10 w-10') }
